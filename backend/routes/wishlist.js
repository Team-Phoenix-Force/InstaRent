...
router.post('/addWish', async (req, res) => {
    try {
        ...
        const existingWishlistItem = await Wishlist.findOne({ userid: userId, id: productId });
        ...
        if (existingWishlistItem) {
            ...
            return res.json({ message: "Product added to wishlist!", status: true, data: savedWishlistItem });
        }
    } catch (error) {
        console.error(error);
        return res.json({ message: "Error adding/removing product to/from wishlist", status: false });
    }
});
...
router.post('/checkWish', async (req, res) => {
    try {
        ...
        const wishlistItem = await Wishlist.findOne({ userid: userId, id: productId });
        ...
        console.error(error);
        return res.json({ message: "Error checking product in wishlist", status: false });
    }
});
